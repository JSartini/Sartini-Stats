...
## Hamiltonian Monte Carlo Cont. {.smaller}
...
- Hamiltonian Dynamics used to propose next state
    ...
    - Describe trajectory with momentum
...
- Step-size resolution $>$ true posterior geometry

- Hamiltonian departs from initial value

    - Total energy (kinetic + potential)
    
- Sampler stops using Hamiltonian dynamics at that step
:::
...
## Accessing Model Outputs (1) {.scrollable}
...
## Accessing Model Outputs (2) {.scrollable}
...
model = cmdstan_model("../resources/codes/Profile_Mod.stan")
fit = model$sample(data = data_list, chains = 1)
```
